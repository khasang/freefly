...
public class BasicDaoImpl<T> implements BasicDao<T> {
    ...
    public List<T> getList() {
        ...
        CriteriaQuery<T> criteraQuery = builder.createQuery(entityClass);
        Root<T> root = criteraQuery.from(entityClass);

        criteraQuery.select(root);

        return getSessionFactory().createQuery(criteraQuery).list();
    }
